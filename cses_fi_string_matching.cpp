...
int main(){
	...
	string s, pattern;
	cin >> s >> pattern;
	rolling_hash hash_s, hash_pattern;
	hash_s.prepare(s);
	hash_pattern.prepare(pattern);
	int x = hash_pattern.get_hash(0, pattern.size() - 1);
	int answer = 0;
	for(int i = 0; i < s.size(); i++){
		if(i + pattern.size() - 1 >= s.size()) break;
		if(hash_s.get_hash(i, i + pattern.size() - 1) == x){
			answer += 1;
		}
	}
	cout << answer << "\n";
